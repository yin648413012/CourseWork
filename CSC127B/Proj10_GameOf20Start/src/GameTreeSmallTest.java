 ...
 * Any method with @BeforeClass executes before any @Test method. This test was written by Rick Mercer
 *
 *---@Test public void testGameWithSevenNodes() {
 * Show how a game could be played by selecting Choice.Yes and Choice.No while;
 * testing the current values of the nodes and asking if an answer was found. This test was written by Rick Mercer
 *
 ...
 * game back to ensure save is working correctly. This test was written by Rick Mercer
 *
 * @author Brian Loi
 */
...
public class GameTreeSmallTest {
	...
	public void testAdd() {
		...
		aGame.add("Does it swim?", "Goose");
		assertEquals("Does it swim?", aGame.getCurrent());
		aGame.toString();
		// Output from toString()
		// - - rattlesnake
		// - Is it a mammal?
	...
	public void testRestart() {
		...
		assertFalse(aGame.foundAnswer());
		Choice userSelection2 = Choice.Yes;
		aGame.playerSelected(userSelection2);
		assertEquals("Barnyard?", aGame.getCurrent());
		assertFalse(aGame.foundAnswer());
		aGame.playerSelected(Choice.No);
		assertEquals("owl", aGame.getCurrent());
	...
	@Test
	public void testSaveGameCatch() {
		GameTree aGame = new GameTree("Empty.txt");
		aGame.saveGame();
	}

	@Test
	public void testNonExistentFile() {
		GameTree aGame = new GameTree("DoesNotExist.txt");
	}
